...
$(document).ready(function(){
  ...
  function adjustProduct(){
  	var groupValue = $("#group_pr").val();
        var tovaru = $("#sprod").val();
  	var tmpSelect = $("#product");
  	if(groupValue.length == 0) {
  		tmpSelect.attr("disabled",true);
  		tmpSelect.clearSelect();
  		/*adjustVulutsya();*/
  	} else {
  		$.getJSON("js/cascadeSelectProd.php",{group:groupValue,tov:tovaru},function(data) { tmpSelect.fillSelect(data).attr("disabled",false); });
  ...
  function adjustVulutsya(){
	...
	var vul_p = $("#idvl").val();
  	...
  	if(rayonValue.length == 0||nspValue.length == 0) {
  		...
  		$.getJSON("../js/cascadeSelectVul2.php",{rayon:rayonValue,nsp:nspValue,vlt:vul_p},function(data) { tmpSelect.fillSelect(data).attr("disabled",false); });
	}
  };
